...
export default function CeoPage() {
    ...
    const fetchData = async () => {
      try {
        const [regionsRes, majorsRes] = await Promise.all([
          axios.get("http://18.141.233.37:4000/api/regions/search?page=1&limit=500", {
  ...
  return (
    <div className="bg-[#f1edfa]"><motion.div
    initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}
    className="relative flex flex-col md:flex-row justify-between items-start md:items-center p-6 min-h-[50vh] text-white bg-cover bg-center mt-30"
    ...
    <p className="text-l md:text-2xl  mt-6 md:mt-0"> {" "}Add Your Education Center</p>
    <p className="text-l md:text-l  mt-20 md:mt-0">Join us and help students discover the best learning opportunities.</p>
    ...
    <div className="flex flex-row w-screen bg-[#f1edfa] justify-center items-center m-0 h-screen my-20 ">
      ...
      <div className="flex h-screen justify-center items-center">
        <motion.img 
          src={ceo} 
          alt="Our Mission" 
          className="max-w-[450px] max-h-[1000px] object-contain ml-[110px] cursor-pointer"
  ...
  {imageFile ? (
    ...
    <label className="block">
      <div className={`flex flex-col items-center justify-center w-full border-2 border-dashed rounded-lg p-5 text-center cursor-pointer transition-colors ${
        errors.image ? "border-red-500 bg-red-50" : "border-gray-300 hover:border-purple-500 hover:bg-purple-50"
      }`}>
        <svg 
...
<Input
  ...
  type="tel"
  value={watch("phone")?.startsWith('+998') 
    ? watch("phone") 
    : `+998${watch("phone") || ""}`}
  onChange={(e) => {
